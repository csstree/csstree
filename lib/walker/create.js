...
function createHandler(config, reverse) {
    return new Function('node', 'context', 'walk',
...
module.exports = function createWalker(config) {
    ...
    for (var name in types) {
        if (hasOwnProperty.call(types, name)) {
            handlers[name] = createHandler(types[name], false);
            handlersReverse[name] = createHandler(types[name], true);
        }
    }
        ...
        if (typeof options === 'function') {
            ...
            if (options.visit) {
                if (visitHandlers.hasOwnProperty(options.visit)) {
                    ...
                    throw new Error('Bad value for `visit` option (should be one of ' + Object.keys(visitHandlers).join(', ') + ')');
                }
