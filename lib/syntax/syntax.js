...
function matchSyntax(dictionary, syntax, value) {
    ...
    if (!result || !result.match) {
        ...
        if (!result || !result.match) {
            dictionary.lastMatchError = MatchError('Mismatch', syntax.syntax, value, unwrapNode(result.next));
            return null;
        }
    }

    if (result.next) {
        dictionary.lastMatchError = MatchError('Uncomplete match', syntax.syntax, value, unwrapNode(result.next));
        return null;
    }
