...
function tokenLayout(scanner, source, startPos) {
    ...
    if (offsets === null || offsets.length < sourceLength + 1) {
        offsets = new LongArray(Math.max(sourceLength + 1024, MIN_ARRAY_SIZE));
        types = new ShortArray(Math.max(sourceLength + 1024, MIN_ARRAY_SIZE));
    }
...
Scanner.prototype = {
    ...
    getLocation: function(offset, source) {
        ...
        var column = line === this.initLine
            ...
            : offset - lastIndexOf.call(this.lines, line - 1, offset);
