...
function getStylesheet(nested) {
    ...
    return {
        type: 'StyleSheet',
        info: getInfo(start, scanner.tokenStart),
        children: children
    };
}
...
function getAtruleExpression() {
    ...
    return {
        ...
        info: getInfo(start, lastNonSpace !== null ? lastNonSpace : scanner.tokenStart),
        children: children
    };
}
    ...
    while (!scanner.eof) {
        ...
        if (combinator !== null) {
            ...
            if (combinator === DESCENDANT_COMBINATOR) {
                combinator = {
                    type: 'Combinator',
                    info: getInfo(combinatorOffset, combinatorOffset + 1),
                    name: ' '
                };
            }
        ...
        case NUMBER:
            switch (scanner.lookupType(1)) {
                ...
                default:
                    return {
                        type: 'Number',
                        info: getInfo(scanner.tokenStart, scanner.tokenEnd),
                        value: readNumber()
                    };
            }
...
function getSelectorFunction(scope, start, name) {
    return getFunctionInternal(function() {
        return new List().appendData(getSelector(NESTED, ABSOLUTE));
    }, scope, start, name);
}

function getRelativeSelectorFunction(scope, start, name) {
    return getFunctionInternal(function() {
...
function getUri(scope, start) {
    ...
    if (scanner.tokenType === STRING) {
        ...
        value = {
            type: 'Raw',
            info: getInfo(rawStart, scanner.tokenStart),
...
function getNthSelector(allowOfClause) {
    ...
    if (scanner.lookupValue(0, 'odd') || scanner.lookupValue(0, 'even')) {
        ...
        query = {
            type: 'Identifier',
            info: getInfo(nthStart, scanner.tokenStart),
    ...
    return {
        ...
        info: getInfo(start, scanner.tokenStart),
        name: name,
        children: new List().appendData({ // TODO: add info
            type: 'Nth',
            nth: query,
            selector: selector
        })
