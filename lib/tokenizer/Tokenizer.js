...
Tokenizer.prototype = {
    ...
    getRawLength: function(endTokenType1, endTokenType2, includeTokenType2) {
        var start = this.currentToken;
        var end = start;
        var max = this.balance[start];

        // search for block-closing token that starts before raw
        while (max < this.tokenCount && this.balance[max] >= start) {
            max = this.balance[max + 1] > max ? this.balance[max + 1] : max + 1;
        }

        if (max >= this.tokenCount) {
            max = this.tokenCount;
        }

        for (; end >= start && end < max; end++) {
            var balance = this.balance[end];
