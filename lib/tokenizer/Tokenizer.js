            ...
            case PUNCTUATOR:
                switch (code) {
                    case balanceCloseCode:
                        var j = balanceStart & OFFSET_MASK;
                        balanceStart = balance[j];
                    ...
                    case LEFTSQUAREBRACKET:
                        balance[offset] = balanceStart;
                        balanceCloseCode = RIGHTSQUAREBRACKET;
                        balanceStart = offset | (balanceCloseCode << TYPE_SHIFT);
                        break;
...
Tokenizer.prototype = {
    ...
    getRawLength: function(start, endTokenType1, endTokenType2, includeTokenType2) {
        var end = this.currentToken;
        var type = 0;
        var sourceLength = this.source.length;
        var max = this.balance[start] || sourceLength;

        // search for block-closing token that starts before raw
        while (max < sourceLength && this.balance[max] >= start) {
            max = this.balance[max + 1] > max ? this.balance[max + 1] : max + 1;
        }

        var offset = start;
        for (; end < this.tokenCount; end++) {
            var type = this.offsetAndType[end] >> TYPE_SHIFT;

            if (offset >= max) {
                break;
            }

            if (this.balance[offset] === 0 || this.balance[offset] === max) {
                if (type === endTokenType1) {
                    break;
                }

                if (type === endTokenType2) {
            ...
            offset = this.offsetAndType[end] & OFFSET_MASK;
        }

        return end - this.currentToken;
    },
