        ...
        switch (type) {
            ...
            case PUNCTUATOR:
                ...
                if (code === LEFTPARENTHESIS && prevType === IDENTIFIER) {
                    // 3 char length identifier and equal to `url` (case insensetive)
                    if (offset - 3 === (tokenCount >= 2 ? offsetAndType[tokenCount - 2] & OFFSET_MASK : startPos) &&
                        cmpStr(source, offset - 3, offset, 'url')) {
                        type = URL;
                        offset = offset + 1;
                        tokenCount--; // rewrite prev token
                        break;
