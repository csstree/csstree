        ...
        case '|':
            ...
            for (var i = terms.length - 1; i >= 0; i--) {
                ...
                if (isEnumCapatible(term)) {
                    ...
                    if (map !== null) {
                        var key = (isFunctionType(term.name) ? term.name.slice(0, -1) : term.type === 'Function' ? term.name + '(' : term.name).toLowerCase();
                        if (key in map === false) {
                            map[key] = term;
                            continue;
                        }
...
module.exports = {
    ...
    buildMatchTree: function(syntax) {
        ...
        if (matchTree !== MISMATCH) {
            matchTree.syntax = ast; // FIXME: prevent mutation of source tree
        }
