...
function internalMatch(tokens, syntax, syntaxes) {
    ...
    while (true) {
        ...
        if (++iterationCount === ITERATION_LIMIT) {
            console.error('[csstree-match] BREAK after ' + ITERATION_LIMIT + ' iterations');
            // console.log(x);
        ...
        switch (syntax.type) {
            ...
            case 'Property':
                ...
                if (hasOwnProperty.call(syntaxes, syntaxDict) && syntaxes[syntaxDict][syntax.name]) {
                    ...
                    console.error('[csstree-match] Unknown <type> reference:', syntax.name);
                    syntax = undefined;
                }
            ...
            default:
                console.error('[csstree-match] Unknown node type:', syntax.type, syntax);
                syntax = MISMATCH;
        }
    }
    ...
    if (syntax === MATCH) {
        ...
        token = longestMatch < tokens.length ? tokens[longestMatch] : null;
...
function matchAsList(tokens, matchTree, syntaxes) {
    var matchResult = internalMatch(tokens, matchTree, syntaxes || {});
...
function matchAsTree(tokens, matchTree, syntaxes) {
    var matchResult = internalMatch(tokens, matchTree, syntaxes || {});
