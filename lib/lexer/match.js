...
function putResult(buffer, match) {
    if (match) {
        // ignore empty non-Node matches,
        // it possible for syntax nodes with `?` or `*` multipliers
        if (match.type !== 'Node' && match.match.length === 0) {
            return;
        }

        if (match.type === 'Group' || match.type === 'Sequence') {
