...
function internalMatch(tokens, syntax, syntaxes = {}) {
    ...
    while (true) {
        ...
        if (syntaxNode === MATCH) {
            ...
            if (syntaxNode.type === 'DisallowEmpty' && ifStack.matchStack === matchStack) {
                syntaxNode = MISMATCH;
                continue;
            }
        ...
        switch (syntaxNode.type) {
            ...
            case 'DisallowEmpty':
                syntaxNode = ifStack === null || matchStack === ifStack.matchStack ? MATCH : MISMATCH;
                break;
            ...
            case 'Property':
                ...
                if (hasOwnProperty.call(syntaxes, syntaxDict) && syntaxes[syntaxDict][syntaxNode.name]) {
                    // console.log(syntaxNode.type, syntaxNode.name, syntaxes[syntaxDict][syntaxNode.name]);
...
function match(ast, matchTree, syntaxes) {
    ...
    const tokens = generate(ast, astToTokenStream);
