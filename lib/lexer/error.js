...
function contains(syntax, node, searchFor) {
    var found = false;

    syntax.walk(node, function(descendant) {
...
var MatchError = function(message, lexer, syntax, value, badNode) {
    ...
    value.children.each(function(node) {
        if (badNode === node || (badNode && contains(lexer.syntax, node, badNode))) {
            errorOffset = css.length;
        }

        css += lexer.syntax.translate(node);
    });
