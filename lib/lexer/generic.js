...
function consumeUntilNodeEnd(token, addTokenToStack, getNextToken) {
    var length = 1;
    var cursor = getNextToken(1);
...
function calc(token, addTokenToStack, getNextToken) {
    ...
    var name = token.node.name.toLowerCase();
    ...
    return consumeUntilNodeEnd(token, addTokenToStack, getNextToken);
}

function attr(token, addTokenToStack, getNextToken) {
    if (!token || token.node.type !== 'Function' || token.node.name.toLowerCase() !== 'attr') {
        return false;
    }
...
function idSelector(token, addTokenToStack, getNextToken) {
    if (!token || token.node.type !== 'IdSelector') {
        return false;
    }
