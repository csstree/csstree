...
var findStringEnd = tokenizerUtils.findStringEnd;
var tokenizerConst = require('../tokenizer/const');
var SYMBOL_TYPE = tokenizerConst.SYMBOL_TYPE;
var IDENT = tokenizerConst.TYPE.Ident;
var PLUSSIGN = tokenizerConst.CHARCODE.PlusSign;
var HYPHENMINUS = tokenizerConst.CHARCODE.HyphenMinus;
var NUMBERSIGN = tokenizerConst.CHARCODE.NumberSign;
var APOSTROPHE = tokenizerConst.CHARCODE.Apostrophe;
var QUOTATIONMARK = tokenizerConst.CHARCODE.QuotationMark;
...
function url(token, addTokenToMatch, getNextToken) {
    if (token === null || token.value.substr(0, 4).toLowerCase() !== 'url(') {
        return false;
    }
...
function consumeIdentifier(str, offset) {
    ...
    if (code < 0x80 && SYMBOL_TYPE[code] !== IDENT && code !== HYPHENMINUS) {
        return offset;
    }
...
function string(token, addTokenToMatch) {
    ...
    if (quote !== APOSTROPHE && quote !== QUOTATIONMARK) {
        return false;
    }

    if (findStringEnd(token.value, 1, quote) !== token.value.length) {
        return false;
    }
