...
function consumeFunction(token, addTokenToMatch) {
...
function declarationValue(token, addTokenToMatch) {
    if (!token) {
        return false;
    }
    ...
    return !token || token.index > startIdx;
}
...
function anyValue(token, addTokenToMatch) {
    if (!token) {
        return false;
    }
