...
var match = require('./match').matchAsTree;
...
function matchSyntax(lexer, syntax, node, useCommon) {
    ...
    if (useCommon) {
        result = match(node, lexer.valueCommonSyntax, lexer);
    }

    if (!useCommon || !result.match) {
        result = match(node, syntax, lexer);
        if (!result.match) {
            return buildMatchResult(null, new MatchError('Mismatch', lexer, syntax.syntax, node, result.tokens, result.error));
        }
    }
