...
var buildMatchTree = require('./match-tree').buildMatchTree;
var matchAsTree = require('./match').matchAsTree;
var trace = require('./trace');
var search = require('./search');
var getStructureFromConfig = require('./structure').getStructureFromConfig;
var cssWideKeywords = buildMatchTree(parse('inherit | initial | unset'));
var cssWideKeywordsWithExpression = buildMatchTree(parse('inherit | initial | unset | <expression>'));
...
function buildMatchResult(match, error, interations) {
    return {
        matched: match,
...
function matchSyntax(lexer, syntax, node, useCommon) {
    ...
    if (useCommon) {
        result = matchAsTree(node, lexer.valueCommonSyntax, lexer);
    }

    if (!useCommon || !result.match) {
        result = matchAsTree(node, syntax.match, lexer);
        if (!result.match) {
            return buildMatchResult(null, new MatchError('Mismatch', lexer, syntax.syntax, node, result.tokens, result.error), result.interations);
        }
    }

    return buildMatchResult(result.match, null, result.interations);
}
    ...
    createDescriptor: function(syntax, type, name) {
        ...
        if (typeof syntax === 'function') {
            ...
            Object.defineProperty(descriptor, 'match', {
                get: function() {
                    Object.defineProperty(descriptor, 'match', {
                        value: buildMatchTree(descriptor.syntax)
                    });
