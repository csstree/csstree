...
describe('generate', () => {
    ...
    describe('auto-emit whitespace', () => {
        it('should auto insert whitespaces where necessary', () => {
            const ast = parse('span#foo { border: 1%var(--a)#ff0000; }');

            assert.strictEqual(generate(ast), 'span#foo{border:1% var(--a) #ff0000}');
        });

        it('should not insert whitespaces for values passed to tokenize() by default', () => {
            const ast = parse('span#foo { border: 1%var(--a)#ff0000; }', {
