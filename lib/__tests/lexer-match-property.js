...
describe('Lexer#matchProperty()', () => {
    ...
    describe('var()', () => {
        ...
        it('should be matched to var() when provided with variable string value', () => {
            const match = lexer.matchProperty('color', parse('var(--foo)', { context: 'value' }), {
                vars: {
                    '--foo': 'red'
                }
        ...
        it('should be matched to var() when provided with variable node value', () => {

            const rule = parse('.rule { --foo: red; color: var(--foo); }', { context: 'rule' });
            const match = lexer.matchProperty('color', rule.block.children.last.value, {
                vars: {
                    '--foo': rule.block.children.first.value.value
                }
            });
        ...
        it('should throw an error when the variable replacement is invalid for the property', () => {
            const match = lexer.matchProperty('color', parse('var(--foo)', { context: 'value' }), {
                vars: {
                    '--foo': 'invalid'
                }
            ...
            assert.strictEqual(match.error.css, 'var(--foo)');
            assert.deepStrictEqual(match.error.loc.start, {
                line: 1,
                column: 1,
                offset: 0
            });
            assert.deepStrictEqual(match.error.loc.end, {
                line: 1,
                column: 11,
                offset: 10
            });

            assert.strictEqual(match.error.mismatchOffset, 0);
            assert.strictEqual(match.error.mismatchLength, 10);
        });
    });
