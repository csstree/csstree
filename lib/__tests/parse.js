...
describe('parse', () => {
    ...
    describe('list', () => {
        it('should use List for children by default', () => {
            const ast = parse('a{}');
            assert(ast.children instanceof List, 'Should be List');
        });

        it('should use List for children when list is true', () => {
            const ast = parse('a{}', { list: true });
            assert(ast.children instanceof List, 'Should be List');
        });

        it('should use Array for children when list is false', () => {
            const ast = parse('a{}', { list: false });
            assert(Array.isArray(ast.children), 'Should be Array');
        });
    });
