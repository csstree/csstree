...
function getAtruleExpression() {
    ...
    while (!scanner.eof) {
        switch (scanner.tokenType) {
            ...
            case WHITESPACE:
                if (child !== null) { // ignore spaces in the beginning
                    wasSpace = true;
                }
        ...
        switch (scanner.tokenType) {
            ...
            default:
                ...
                if (scanner.tokenType === IDENTIFIER) {
                    if (scanner.lookupValue(0, 'u') && scanner.lookupType(1) === PLUSSIGN) {
                        var info = getInfo();
                        var start = scanner.tokenStart;
...
function readAttrselector() {
    scanner.expectAny('Attribute selector (=, ~=, ^=, $=, *=, |=)',
        EQUALSSIGN,        // =
        TILDE,             // ~=
        CIRCUMFLEXACCENT,  // ^=
        DOLLARSIGN,        // $=
    ...
    if (scanner.tokenType === EQUALSSIGN) {
        scanner.next();
    } else {
        scanner.next();
...
function getCombinator() {
    ...
    switch (scanner.tokenType) {
        ...
        case SOLIDUS:
            ...
            scanner.expectIdentifier('deep', true);
...
function tryGetPercentage() {
    ...
    if (number !== null && scanner.tokenType === PERCENTSIGN) {
        return getPercentage(number);
    }
...
function getFunction(scope, info, name) {
    ...
    if (scope && scope.hasOwnProperty(nameLowerCase)) {
        return scope[nameLowerCase](scope, info, name);
    }
...
function getImportant() {
    ...
    scanner.expectIdentifier('important', true);
...
function getVhash() {
    ...
    scanner.expectAny('Number or identifier',
        DECIMALNUMBER,
        IDENTIFIER
    );
